...
struct BusinessProfileView2_Previews: PreviewProvider {
    static var previews: some View {
        @Environment(\.router) var router
        UserProfileView2(viewModel: UserProfileViewModel(router: router))
            .environmentObject(LocalizationManager.shared)
    }
}

// MARK: - Functions
extension UserProfileView2 {
    ...
    private var amenitiesSection: some View {
        VStack(alignment: .leading, spacing: 4) {
            ...
            ScrollView(.horizontal, showsIndicators: false) {
                LazyHGrid(rows: viewModel.profileData?.businessProfileRef?.amenitiesRef?.count ?? 0 < 5 ? singleColumn : amenitiesColumns, spacing: 4) {
        ...
        VStack {
            if viewModel.userIsIndividual {
                Circle()
                    ...
                    .overlay {
                        WebImage(url: URL(string: viewModel.profileData?.profilePic?.small ?? ""), content: { image in
                            image
                                .resizable()
                                .scaledToFill()
                                .clipShape(Circle())
                ...
                Circle()
                    ...
                    .frame(width: 56, height: 56)
                    .overlay(
                        ProfileProgressCircle(progress: .constant(viewModel.profileData?.profileCompleted ?? 0))
                            .stroke(lineWidth: 2)
                            .fill(.green)
                            ...
                            .overlay(
                                Circle()
                                    ...
                                    .overlay(
                                        WebImage(
                                            url: URL( string: viewModel.profileData?.businessProfileRef?.profilePic?.small ?? "" )
                                        )
                                            .resizable()
                                            .scaledToFill()
                                            .frame(width: 52, height: 52)
