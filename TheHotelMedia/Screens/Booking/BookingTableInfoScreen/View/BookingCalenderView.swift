...
struct BookingCalenderView: View {
    ...
    @ObservedObject var viewModel: BookingCalenderViewModel
//    @Binding var startDate: String
//    @Binding var endDate: String
    @Binding var calendarType: CalendarType
    var onSelectDate: ((String?) -> Void)? = nil
    var onTapPastDay: (() -> Void)? = nil
        ...
        VStack {
            ...
            if viewModel.calendarType == .days {
                HStack(alignment: .bottom) {
                    Text("date".localized(localizationManager.language))
...
//                }
        ...
        .onAppear {
            onSelectDate?("\(viewModel.currentYear)-\(viewModel.currentMonth)-\(viewModel.currentDay)")
        }
    }
}
...
extension BookingCalenderView {
    ...
    func isPastDay(day: Int, month: Int, year: Int) -> Bool {
        ...
        guard let inputDate = calendar.date(from: components) else {
            return false
        }
...
//        .foregroundColor(.white.opacity(viewModel.selectedDay == day.date ? 1.0 : 0.5))
        ...
        .onTapGesture {
            ...
            if !isPastDay(day: date, month: month, year: year) {
                haptics(.light)
...
//                switch day.monthType {
//                case .current:
//                    onSelectDate?("\(viewModel.year)-\(viewModel.month)-\(day.date)")
//                case .previous:
//                    if viewModel.month == 1 {
//                        onSelectDate?("\(viewModel.year - 1)-\(12)-\(day.date)")
//                    } else {
//                        onSelectDate?("\(viewModel.year)-\(viewModel.month - 1)-\(day.date)")
//                    }
//                case .next:
//                    if viewModel.month == 12 {
//                        onSelectDate?("\(viewModel.year + 1)-\(1)-\(day.date)")
//                    } else {
//                        onSelectDate?("\(viewModel.year)-\(viewModel.month + 1)-\(day.date)")
//                    }
//                }
...
//        .scaleEffect(y: viewModel.bookTableMnYDropDownOpen ? 1 : 0, anchor: .top)
    ...
    private var monthTypeSection: some View {
        VStack(spacing: 6) {
            if viewModel.year == viewModel.currentYear {
                ForEach(viewModel.currentMonth...12) { index in
                    if let month = getMonth(int: index) {
                        monthTypeButton(type: month)
                    }
                }
            } else if viewModel.year > viewModel.currentYear {
    ...
    private var yearTypeSection: some View {
        VStack(spacing: 6) {
            ForEach(0..<5) { index in
                yearTypeButton(year: viewModel.currentYear + index)
            }
