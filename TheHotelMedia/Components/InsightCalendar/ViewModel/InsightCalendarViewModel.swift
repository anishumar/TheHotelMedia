//
//  InsightCalendarViewModel.swift
//  HotelMedia
//
...
enum THMCalendarType {
    case days, full
}


final class InsightCalendarViewModel: ObservableObject {
        ...
        if let indexOfFirstDay = weekDaysShortForm.firstIndex(where: {$0 == array.first?.day}) {
            if let daysInPreviousMonth = daysInMonth(month: month == 1 ? 12 : month - 1, year: month == 1 ? year - 1 : year) {
                for i in 0..<indexOfFirstDay {
                    array.insert(Day(day: weekDaysShortForm[i], date: daysInPreviousMonth - (indexOfFirstDay - (i + 1)), monthType: .previous), at: i)
                }
            }
        }
